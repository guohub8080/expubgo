...
import { isNil, isString, isNumber, isDate, isPlainObject, isFunction } from 'es-toolkit/predicate';
...
const getDayjs = (dayInput?: string | number | Date | Dayjs): Dayjs => {
    ...
    if (!isDate(dayInput) && !isString(dayInput) && !isNumber(dayInput) && isFunction((dayInput as unknown as { format?: unknown }).format)) {
