/**
 * SvgComponentsDoc SVG组件文档动态加载器
 * 自动扫描 src/books/SvgComponentsDoc 文件夹下的所有 .tsx, .md, .mdx 文件
 * 按文件夹分组，每个文件夹对应一个分类
 *
 * 文件夹命名规则：[x]M
 * - [x] - 排序号，数字越小越靠上
...
import { svgComponentsDocConfig } from './info.tsx';
...
// 当前文件: src/books/SvgComponentsDoc/data/svgComponentsDocLoader.tsx
// 目标目录: src/books/SvgComponentsDoc/**/*.{tsx,md,mdx}
...
function groupAndSortDocuments(): DocumentCategory[] {
  ...
  Object.entries(documentModules).forEach(([path, module]) => {
    ...
    if ((fileType === 'mdx' || fileType === 'md') && module.title) {
      (document as { title?: unknown }).title = module.title;
    }
    ...
    const fullUrl = `${svgComponentsDocConfig.slug}/${routePath}`;
...
// 导出分组后的文档数据
export const svgComponentsDocCategories = groupAndSortDocuments();

// 导出扁平化的所有文档（用于路由）
export const allSvgComponentsDocs = svgComponentsDocCategories.flatMap(cat => cat.articles);

// 创建 BookLoader 实现
const svgComponentsDocLoader: BookLoader = {
  config: {
    title: svgComponentsDocConfig.title,
    slug: svgComponentsDocConfig.slug,
    icon: () => svgComponentsDocConfig.icon,
    description: svgComponentsDocConfig.description
  },

  categories: svgComponentsDocCategories,

  getAllArticles(): BookArticle[] {
    return allSvgComponentsDocs;
  },

  getCurrentArticle(path: string): BookArticle | null {
    const normalizedPath = path.toLowerCase().replace(/^\/+|\/+$/g, '');
    return allSvgComponentsDocs.find(article =>
  ...
  getNavigationData() {
    return {
      categories: svgComponentsDocCategories,
      allArticles: allSvgComponentsDocs
    };
  },
...
// 生成路由的函数
export function generateSvgComponentsDocRoutes(): RouteObject[] {
  return [
    {
      path: svgComponentsDocConfig.slug,
      element: (
        <BookLayoutConfigProvider basePrefix={`/${svgComponentsDocConfig.slug}`}>
          <BookLayout loader={svgComponentsDocLoader} />
      ...
      children: [
        ...
        {
          index: true,
          element: allSvgComponentsDocs.length > 0
            ? (<DebugRedirect to={allSvgComponentsDocs[0].routePath} />)
            : <div className="p-6 text-center text-muted-foreground">暂无文章</div>,
        },
        // 所有组件页面 - 用 ArticleContent 包装
        ...svgComponentsDocCategories.flatMap((category) =>
          category.articles.map((article) => {
            // 子路由路径（相对于 /{slug}）
            const path = article.routePath;
            return {
              path: path,
              element: <ArticleContent loader={svgComponentsDocLoader}>{article.content}</ArticleContent>,
            };
          })
        ),
      ],
...
export default svgComponentsDocLoader;
